...
function applyProjectBackgrounds() {
  ...
  projects.forEach(project => {
    // Get current theme variable values
    const styles = getComputedStyle(document.body);
    const circles = styles.getPropertyValue("--card-bg-circles").trim();
    const svgs = [
      `<svg viewBox="0 0 900 900" xmlns="http://www.w3.org/2000/svg">
        <g fill="${circles}">
          <circle r="181" cx="326" cy="60"/>
          <circle r="79"  cx="538" cy="732"/>
          <circle r="122" cx="27"  cy="789"/>
        </g>
      </svg>`,

      `<svg viewBox="0 0 900 900" xmlns="http://www.w3.org/2000/svg">
        <g fill="${circles}">
          <circle r="181" cx="875" cy="456"/>
          <circle r="79"  cx="546" cy="75"/>
          <circle r="115" cx="229" cy="358"/>
          <circle r="138" cx="90"  cy="869"/>
        </g>
      </svg>`,

      `<svg viewBox="0 0 900 900" xmlns="http://www.w3.org/2000/svg">
        <g fill="${circles}">
          <circle r="181" cx="87"  cy="731"/>
          <circle r="79"  cx="529" cy="830"/>
          <circle r="104" cx="845" cy="339"/>
        </g>
      </svg>`
    ];

    const randomSvg = svgs[Math.floor(Math.random() * svgs.length)];
    const encoded = encodeURIComponent(randomSvg).replace(/'/g, "%27").replace(/"/g, "%22");

    project.style.backgroundImage = `url("data:image/svg+xml,${encoded}")`;
  });
}
document.addEventListener('DOMContentLoaded', applyProjectBackgrounds);
const themeObserver = new MutationObserver(() => applyProjectBackgrounds());
